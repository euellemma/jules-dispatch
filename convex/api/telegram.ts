...
async function telegramApiCall(endpoint: string, body: object): Promise<any> {
  ...
  return await withRetry(async () => {
    ...
    if (!response.ok) {
      ...
      const error: any = new Error(`Telegram API error: ${response.status} ${err}`);
...
async function sendTelegramChatAction(chatId: string, action: "typing" | "cancel"): Promise<void> {
...
async function sendTelegramDocument(
  ...
  formData.append("document", new Blob([new Uint8Array(fileBuffer).buffer]) as any, filename);
...
export async function processTelegramUpdate(
  ctx: any,
  update: any
): Promise<{ success: boolean; handled?: boolean; threadId?: string; error?: string }> {
  try {
    ...
    if (message.document) {
      ...
      if (size > maxSizeBytes) {
        await sendTelegramMessage(chatId, `File "${fileName}" exceeds the 20MB limit.`);
        return { success: true, handled: true };
      }

      const threadId = await ctx.runMutation(internal.users.db.getOrCreateUserThread, {
        telegramChatId: chatId,
      });

      const fileBuffer = await downloadTelegramFile(fileId);
      const storageId = await ctx.storage.store(new Blob([new Uint8Array(fileBuffer).buffer]) as any);
    ...
    captureException(error, { tags: { handler: "processTelegramUpdate" }, extra: { updateId: update.update_id } });
...
async function handleTelegramCommand(
  ...
  switch (command) {
    ...
    case "/connect": {
      ...
      const siteUrl = process.env.CONVEX_SITE_URL || "https://aware-pheasant-429.convex.site";
    ...
    case "/new":
      await sendTelegramMessage(chatId, "🆕 <b>Starting fresh...</b> Wiping todos and files, but keeping our shared history.");
    ...
    case "/reset":
      await telegramApiCall("sendMessage", {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              { text: "☢️ Yes, Nuke Everything", callback_data: "nuke_confirm" },
...
async function processTelegramCallbackQuery(ctx: any, query: any) {
  ...
  if (data === "nuke_confirm") {
    ...
    await sendTelegramMessage(chatId, "✅ <b>System Reset Complete.</b> All memories and threads have been deleted. API keys preserved.");
...
export const processMessageQueue = internalAction({
  ...
  handler: async (ctx: any, { threadId, telegramChatId }: { threadId: string; telegramChatId: string }) => {
    const isRunning = await ctx.runQuery(internal.users.db.isAgentRunning, { threadId });
    if (isRunning) {
    ...
    const pendingText = await ctx.runQuery(internal.users.db.getPendingMessages, { threadId });
    ...
    try {
      ...
      const messages = pendingText.split("\n").filter((m: string) => m.trim() !== "");

      // We DON'T clear messages yet. We clear them one by one as they succeed.
      // This enables multi-turn recovery.

      for (const message of messages) {
        console.log("[processMessageQueue] Processing message:", message.substring(0, 50));

        const model = await resolveLanguageModel(ctx, threadId);
        await julesAgent.generateText(ctx, { threadId }, { model, prompt: message });
        
        // Success! Remove ONLY this message from the pending text
        await ctx.runMutation(internal.users.db.appendPendingMessage, { 
          threadId, 
          text: "", // placeholder for a more complex "remove first line" mutation
          // Refactor note: For simplicity in this POC, we'll clear all if successful, 
          // but true multi-turn recovery would pop from a queue.
        });
      }

      await ctx.runMutation(internal.users.db.clearPendingMessages, { threadId });
      ...
      let tip = "<i>You can change your AI provider or model by using /connect. Your message is saved and will resume once you update your settings.</i>";
      
      if (errorMessage.includes("Provider not configured") || errorMessage.includes("API key")) {
        tip = "<b>Tip:</b> Your AI provider might be misconfigured. Use /connect to check your settings.";
      ...
      const newPending = await ctx.runQuery(internal.users.db.getPendingMessages, { threadId });
...
export const sendChatDocument = internalAction({
  ...
  handler: async (ctx: any, args: { chatId: string, fileContent: string, filename: string, caption?: string }) => {
    try {
...
export const downloadAndStoreFile = internalAction({
  ...
  handler: async (ctx: any, args: { telegramChatId: string, fileId: string, fileName: string, caption?: string, size: number }) => {
    try {
      ...
      try {
        ...
        console.error("[downloadAndStoreFile] Failed to send error message:", e);
      }
    }
  },
});

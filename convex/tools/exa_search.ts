...
const researchInstructions = (
  ...
  (injectedContext
    ? `### INJECTED CONTEXT (FILE CONTENTS)\n${injectedContext}\n\n`
    : "") +
...
async function runResearchAgent(
  ...
  console.log(
    `[research] Starting ${mode} research agent for: "${query.slice(0, 60)}..." (maxSteps=${maxSteps})`,
  );
  ...
  try {
    ...
    const suffix =
      mode === "quick" && (result as any).finishReason === "maxSteps"
        ? `\n\n[research may be incomplete — consider using deep research for more]`
        : "";

    console.log(
      `[research] Completed ${mode} research for: "${query.slice(0, 60)}..."`,
    );
    return `### Research/Analysis: "${query}"\n\n${result.text}${suffix}`;
  } catch (error: any) {
    console.error(
      `[research] Error during ${mode} research:`,
...
export const research = createTool({
  description:
    "Delegate tasks to the Research Agent. Use 'quick' for fast answers, 'deep' for thorough multi-step research. You can optionally inject files from uploaded files (by assignedName) or from a Jules session (by file paths).",
  inputSchema: z.object({
    query: z
      .string()
      .describe("The research query, question, or analysis instruction."),
    mode: z
      .enum(["quick", "deep"])
      .optional()
      .default("quick")
      .describe("Quick for fast answers, deep for thorough research."),
    files: z
      .array(
        z.discriminatedUnion("type", [
          ...
          z.object({
            ...
            filePaths: z
              ...
              .describe(
                "Single file path or array of file paths from this session.",
              ),
          }),
          ...
          z.object({
            ...
            names: z
              .array(z.string())
              .describe(
                "Array of file assignedNames from your Registered Files list.",
              ),
          }),
        ]),
      )
      .optional()
      .describe(
        "Files to inject into research context. Can mix session files and uploaded files.",
      ),
  }),
  execute: async (ctx, args) => {
    try {
      ...
      console.log(
        `[research] Starting research: "${args.query.slice(0, 80)}..." (mode=${args.mode})`,
      );
      ...
      if (args.files && args.files.length > 0) {
        for (const fileSpec of args.files) {
          if (fileSpec.type === "uploaded") {
            ...
            for (const name of fileSpec.names) {
              try {
                const fileInfo = await ctx.runQuery(
                  (internal as any).files.db.getFileByAssignedName,
                  {
                    threadId: ctx.threadId,
                    assignedName: name,
                  },
                ...
                if (!response.ok) {
                  missingFiles.push(
                    `${name} (fetch failed: ${response.statusText})`,
                  );
                  continue;
                }
            ...
            const rawOutputs = await ctx.runQuery(
              internal.sessions.db.getSessionOutputsRaw,
              {
                julesSessionId: fileSpec.julesSessionId,
              },
            );

            // Fetch content from Convex storage
      ...
      // Run research
      console.log(
        `[research] Running ${args.mode} research with ${foundFiles.length} file(s) in context`,
      );
      ...
      if (missingFiles.length > 0) {
        console.warn(
          `[research] ${missingFiles.length} file(s) missing: ${missingFiles.join(", ")}`,
        );
